<template>
  <div class="old-house-page">
    <van-nav-bar title="二手房" fixed placeholder>
      <template #right>
        <FallbackLink :href="FALLBACK_URLS.oldHouse" label="原站" />
      </template>
    </van-nav-bar>
    ...
    <div class="page-content">
      ...
      <EmptyState
        ...
        message="暂无匹配的二手房数据"
        :fallback-url="FALLBACK_URLS.oldHouse"
      />
...
function getDefaultFilter(): OldHouseFilter {
  return { page: 1, pageSize: 10 };
}

function getFilterFromQuery(): OldHouseFilter {
  const params = new URLSearchParams(route.query as Record<string, string>);
  // Enforce page type: a URL with type=new-house on the old-house route must
  // be parsed as an OldHouseFilter so foreign fields (projectName, status)
  // cannot leak into the filter state.
  params.set('type', 'old-house');
  const deserialized = deserializeFilter(params) as OldHouseFilter;
  return { ...getDefaultFilter(), ...deserialized };
}

const filter = ref<OldHouseFilter>(getFilterFromQuery());
...
async function doSearch(f: OldHouseFilter) {
  if (abortController) {
  ...
  try {
    ...
    if (f.minPrice !== undefined) body.minPrice = f.minPrice;
    if (f.maxPrice !== undefined) body.maxPrice = f.maxPrice;
    if (f.rooms !== undefined) body.rooms = f.rooms;
...
async function fetchCaptcha() {
  ...
  try {
    const result = await apiGet<CaptchaData>('/api/captcha?purpose=old-house', signal);
...
function onFilterApply(newFilter: OldHouseFilter) {
...
function onRemoveChip(key: string) {
  ...
  if (key === 'minPrice') f.minPrice = undefined;
  if (key === 'maxPrice') f.maxPrice = undefined;
  if (key === 'rooms') f.rooms = undefined;
  if (key === 'keyword') f.keyword = undefined;
  f.page = 1;
  clearCaptcha();
  updateQuery(f);
  doSearch(f);
...
function onDetailClick(id: string) {
  router.push(`/old-house/${id}`);
}

// ── URL sync ────────────────────────────────────────────────────────────────

function updateQuery(f: OldHouseFilter) {
...
watch(
  ...
  (newQuery) => {
    ...
    if (deserialized.page !== filter.value.page ||
        ...
        deserialized.minPrice !== filter.value.minPrice ||
        deserialized.maxPrice !== filter.value.maxPrice ||
        deserialized.rooms !== filter.value.rooms ||
        deserialized.propertyType !== filter.value.propertyType ||
        deserialized.keyword !== filter.value.keyword) {
